...
namespace RoyalCourtServants
{
    ...
    public class RoyalCourt
    {
        ...
        public double GetAverageProductivity()
        {
            ...
            servants.ForEach(s => { productivitySum += s.Productivity; servantsCounter++; });

            var averageProductivity = productivitySum / servantsCounter;
        ...
        {
            int minimumReliability = int.MaxValue;
            string servantName = default;
            string servantRole = default;

            foreach (var servant in servants)
            {
                if (servant.Reliability < minimumReliability)
                {
                    minimumReliability = servant.Reliability;
                    servantName = servant.Name;
                    servantRole = servant.Role;
                }
            }

            return $"The servant {servantName} with role -- {servantRole} -- has the minimum reliability with {minimumReliability} %";
        }

        public string GetMaximumReliability()
        {
            int maximumReliability = int.MinValue;
            string servantName = default;
            string servantRole = default;

            foreach (var servant in servants)
            {
                if (servant.Reliability > maximumReliability)
                {
                    maximumReliability = servant.Reliability;
                    servantName = servant.Name;
                    servantRole = servant.Role;
                }
            }

            return $"The servant {servantName} with role -- {servantRole} -- has the maximum reliability with {maximumReliability} %";
        }
        ...
        public string ToBePromoted()
        {
            ...
            foreach (var servant in servants)
            {
                ...
                {
                    promotionList += $"{servant.Name} with role {servant.Role} will be promoted with new wage of {servant.Wage + 50}\n";
                }
            }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"The average productivity for all servants is : {royalCourt.GetAverageProductivity().ToString("N2")}");
